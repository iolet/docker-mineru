...
def read_fn(path):
    ...
    with open(str(path), "rb") as input_file:
        ...
        if file_suffix in ["png", "jpeg", "jp2", "webp", "gif", "bmp", "jpg", "tiff"]:
...
def _prepare_pdf_bytes(pdf_bytes_list, start_page_id, end_page_id):
    ...
    for pdf_bytes in pdf_bytes_list:
        new_pdf_bytes = _convert_pdf_bytes_to_bytes_by_pypdfium2(pdf_bytes, start_page_id, end_page_id)
...
def _process_vlm(
    ...
    if 'vlm_doc_analyze' not in globals():
...
def _process_hybrid(
    ...
    if 'hybrid_doc_analyze' not in globals():
